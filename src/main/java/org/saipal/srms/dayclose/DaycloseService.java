...
//		System.out.println(lgid);
		...
		List<Tuple> admlvl = db.getResultList(sql, Arrays.asList(auth.getBankId(), auth.getBranchId(), auth.getBankId(),
				auth.getBranchId(), auth.getBankId(), auth.getBranchId()));
...
//		System.out.println(cb);
		...
		if (jarr.length() > 0) {
			for (int i = 0; i < jarr.length(); i++) {
				...
				Map<String, Object> t = db.getSingleResultMap(sq,
						Arrays.asList(parts[0], parts[1], auth.getBranchId()));
...
//					System.out.println(cb.get(parts[1]));
					...
					DbResponse rowEffect = db.execute(sql,
							Arrays.asList(id, parts[0], parts[1], parts[2], parts[3], parts[4], parts[5],
									auth.getBankId(), auth.getBranchId(), auth.getUserId(), cb.get(parts[1])));
//					System.out.println(rowEffect.getMessage());
					...
					if (!admlvl.isEmpty()) {
						for (Tuple tt : admlvl) {
							...
							db.execute(sql2, Arrays.asList(id, tt.get("id"), tt.get("karobarsanket"),
									...
									tt.get("collectioncenterid"), tt.get("bankid"), tt.get("branchid"),
					...
					if (!t1.isEmpty()) {
						for (Tuple tt : t1) {
							...
							db.execute(sq2, Arrays.asList(id, tt.get("id"), tt.get("karobarsanket"),
									tt.get("voucherno"), tt.get("date"), tt.get("taxpayername"), tt.get("taxpayerpan"),
									tt.get("depositedby"), tt.get("depcontact"), tt.get("lgid"),
									tt.get("collectioncenterid"), tt.get("bankid"), tt.get("branchid"),
									tt.get("bankorgid"), tt.get("purpose"), tt.get("syncstatus"), tt.get("approved"),
...
//					System.out.println("i am here");
						...
						for (Tuple tt : t3) {
							...
							db.execute(sql4,
									Arrays.asList(id, tt.get("id"), tt.get("transactionid"), tt.get("bankvoucherno"),
											...
											0, 0, tt.get("usestatus"), 0, 0, tt.get("amount")));
						}
					}
	...
	/**
	 * Performs Dayclose for Earlier Today
	 * 
	 * @Return Void
	 */
	...
	public ResponseEntity<Map<String, Object>> daycloseScheduler() {
		String sql = "select *,(amountcr-amountdr) as balance from (select accountno,bankid,depositbranchid,accountname,accountnumber,palika,lgid,sum(amountcr) as amountcr,sum(amountdr) as amountdr from ("
				+ " select  cast(t.bankorgid as varchar) as accountno,t.depositbranchid,t.bankid,b.accountname,b.accountnumber,ll.namenp as palika,cast(t.lgid as varchar) as lgid,t.amountcr, t.amountdr from taxvouchers t join admin_local_level_structure ll on ll.id=t.lgid join bankaccount b on b.id=t.bankorgid left join dayclose dc on dc.lgid=t.lgid and dc.bankorgid=t.bankorgid and (dc.dateint=t.dateint or dc.dateint=t.cleardateint)    where  dc.id is null and (t.dateint=format(dateadd(day,-1,getdate()),'yyyyMMdd') or t.cleardateint=format(dateadd(day,-1,getdate()),'yyyyMMdd'))   and ( t.approved=1 or t.cstatus=1) "
				+ " union all "
				+ " select  cast(t.bankorgid as varchar) as accountno,t.depositbranchid,t.bankid,b.accountname,b.accountnumber,ll.namenp as palika,cast(t.lgid as varchar) as lgid,t.amountcr, t.amountdr from taxvouchers_log t join admin_local_level_structure ll on ll.id=t.lgid join bankaccount b on b.id=t.bankorgid  left join dayclose dc on dc.lgid=t.lgid and dc.bankorgid=t.bankorgid and dc.dateint=t.dateint    where  dc.id is null and  t.dateint=format(dateadd(day,-1,getdate()),'yyyyMMdd')   and (t.approved=1 or t.cstatus=1) "
				+ " union all "
...
//						System.out.println("deatils done");
					...
					List<Tuple> t1 = db.getResultList(sq1,
							Arrays.asList(t.get("lgid"), t.get("accountno"), t.get("depositbranchid")));
					...
					String sql3 = "select * from bank_deposits where lgid=? and bankorgid=? and paymentmethod=? and depositbranchid=? and approved=1 and depositdateint=format(dateadd(day,-1,getdate()),'yyyyMMdd')";
					List<Tuple> t3 = db.getResultList(sql3,
							Arrays.asList(t.get("lgid"), t.get("accountno"), 2, t.get("depositbranchid")));
...
//		System.out.println(lgid);
		...
		String sql = "select *,(amountcr-amountdr) as balance from (select accountno,bankid,depositbranchid,accountname,accountnumber,palika,lgid,sum(amountcr) as amountcr,sum(amountdr) as amountdr from ("
				+ " select  cast(t.bankorgid as varchar) as accountno,t.depositbranchid,t.bankid,b.accountname,b.accountnumber,ll.namenp as palika,cast(t.lgid as varchar) as lgid,t.amountcr, t.amountdr from taxvouchers t join admin_local_level_structure ll on ll.id=t.lgid join bankaccount b on b.id=t.bankorgid left join dayclose dc on dc.lgid=t.lgid and dc.bankorgid=t.bankorgid and (dc.dateint=t.dateint or dc.dateint=t.cleardateint) and dc.branchid="
				+ branchid + "  where  dc.id is null   and  t.bankid=?  and t.branchid=?  and (t.dateint= " + dateint
				+ " or t.cleardateint= " + dateint + ")   and ( t.approved=1 or t.cstatus=1) " + cond + " union all "
				+ " select  cast(t.bankorgid as varchar) as accountno,t.depositbranchid,t.bankid,b.accountname,b.accountnumber,ll.namenp as palika,cast(t.lgid as varchar) as lgid,t.amountcr, t.amountdr from taxvouchers_log t join admin_local_level_structure ll on ll.id=t.lgid join bankaccount b on b.id=t.bankorgid  left join dayclose dc on dc.lgid=t.lgid and dc.bankorgid=t.bankorgid and dc.dateint=t.dateint and dc.branchid="
				+ branchid + "   where  dc.id is null and  t.dateint= " + dateint
				+ " and  t.bankid=? and t.branchid=? and ( t.approved=1 or t.cstatus=1) " + cond + " union all "
				+ " select  cast(t.bankorgid as varchar) as accountno,t.depositbranchid,t.bankid,b.accountname,b.accountnumber,ll.namenp as palika,cast(t.lgid as varchar) as lgid,t.amount as amountcr,0 as  amountdr from bank_deposits t join admin_local_level_structure ll on ll.id=t.lgid join bankaccount b on b.id=t.bankorgid left join dayclose dc on dc.lgid=t.lgid and dc.bankorgid=t.bankorgid and dc.dateint=t.depositdateint and dc.branchid="
				+ branchid + "   where  dc.id is null and  t.depositdateint= " + dateint
				+ " and  t.bankid=?  and t.depositbranchid=? and t.approved=1 " + cond1
				+ " ) a group by accountno,accountname,accountnumber,palika,lgid,bankid,depositbranchid) b ";
...
//		System.out.println(sql);
		...
		if (!admlvl.isEmpty()) {
			for (Tuple t : admlvl) {
				String sq = "select count(id) as cid from dayclose where lgid=? and accountno=? and branchid=? and dateint="
						+ dateint;
				Map<String, Object> ttt = db.getSingleResultMap(sq,
						Arrays.asList(t.get("lgid"), t.get("accountnumber"), t.get("depositbranchid")));
				if (ttt.get("cid").toString().equals("0")) {
					String id = db.newIdInt();
					String sql1 = "insert into dayclose(id,lgid,bankorgid,accountno,accountname,amountdr,amountcr,bankid,branchid,creatorid,corebankid,dateint) values (?,?,?,?,?,?,?,?,?,?,?,?) ";
					DbResponse rowEffect = db.execute(sql1,
							Arrays.asList(id, t.get("lgid"), t.get("accountno"), t.get("accountnumber"),
									t.get("accountname"), t.get("amountdr"), t.get("amountcr"), t.get("bankid"),
									t.get("depositbranchid"), 0, "system", dateint));

					String sql2 = "select * from taxvouchers where lgid=? and bankorgid=? and branchid=?  and (approved=1 or cstatus=1) and (dateint= "
							+ dateint + " or cleardateint= " + dateint + ")";
					List<Tuple> txv = db.getResultList(sql2,
							Arrays.asList(t.get("lgid"), t.get("accountno"), t.get("depositbranchid")));

					if (!txv.isEmpty()) {
						for (Tuple tt : txv) {
							String sql3 = "insert into dayclose_details(dcid,tvid,karobarsanket,dateint,voucherno,date,taxpayername,taxpayerpan,depositedby,depcontact,lgid,collectioncenterid,bankid,branchid,accountno,purpose,syncstatus,approved,approverid,ttype,chequebank,chequeno,chequeamount,cstatus,chequetype,isused,hasChangeReqest,changeReqestDate,amountdr,amountcr) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NULL,?,?) ";
							db.execute(sql3, Arrays.asList(id, tt.get("id"), tt.get("karobarsanket"), dateint,
									tt.get("voucherno"), tt.get("date"), tt.get("taxpayername"), tt.get("taxpayerpan"),
									tt.get("depositedby"), tt.get("depcontact"), tt.get("lgid"),
									tt.get("collectioncenterid"), tt.get("bankid"), tt.get("branchid"),
									tt.get("bankorgid"), tt.get("purpose"), tt.get("syncstatus"), tt.get("approved"),
					...
					String sq1 = "select * from taxvouchers_log where lgid=? and bankorgid=? and branchid=?  and (approved=1 or cstatus=1) and dateint= "
							+ dateint;
					List<Tuple> t1 = db.getResultList(sq1,
							Arrays.asList(t.get("lgid"), t.get("accountno"), t.get("depositbranchid")));
					...
					if (!t1.isEmpty()) {
						for (Tuple tt : t1) {
							String sq2 = "insert into dayclose_details(dcid,tvid,karobarsanket,dateint,voucherno,date,taxpayername,taxpayerpan,depositedby,depcontact,lgid,collectioncenterid,bankid,branchid,accountno,purpose,syncstatus,approved,approverid,ttype,chequebank,chequeno,chequeamount,cstatus,chequetype,isused,hasChangeReqest,changeReqestDate,amountdr,amountcr) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NULL,?,?) ";
							db.execute(sq2, Arrays.asList(id, tt.get("id"), tt.get("karobarsanket"), dateint,
									tt.get("voucherno"), tt.get("date"), tt.get("taxpayername"), tt.get("taxpayerpan"),
									tt.get("depositedby"), tt.get("depcontact"), tt.get("lgid"),
									tt.get("collectioncenterid"), tt.get("bankid"), tt.get("branchid"),
									tt.get("bankorgid"), tt.get("purpose"), tt.get("syncstatus"), tt.get("approved"),
					...
					String sql3 = "select * from bank_deposits where lgid=? and bankorgid=? and paymentmethod=? and depositbranchid=? and approved=1 and depositdateint= "
							+ dateint;
					List<Tuple> t3 = db.getResultList(sql3,
							Arrays.asList(t.get("lgid"), t.get("accountno"), 2, t.get("depositbranchid")));
					...
					if (!t3.isEmpty()) {
						for (Tuple tt : t3) {
							String sql4 = "insert into dayclose_details(dcid,tvid,karobarsanket,dateint,voucherno,date,taxpayername,taxpayerpan,depositedby,depcontact,lgid,collectioncenterid,bankid,branchid,accountno,purpose,syncstatus,approved,approverid,ttype,chequebank,chequeno,chequeamount,cstatus,chequetype,isused,hasChangeReqest,changeReqestDate,amountdr,amountcr) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NULL,?,?) ";
							db.execute(sql4,
									Arrays.asList(id, tt.get("id"), tt.get("transactionid"), dateint,
											tt.get("bankvoucherno"), tt.get("voucherdate"), tt.get("taxpayername"), 0,
											tt.get("taxpayername"), tt.get("mobileno"), tt.get("lgid"),
											tt.get("collectioncenterid"), tt.get("bankid"), tt.get("depositbranchid"),
											tt.get("bankorgid"), "", tt.get("syncstatus"), tt.get("approved"),
											tt.get("approverid"), 1, 0, "", 0, 0, 0, tt.get("usestatus"), 0, 0,
											tt.get("amount")));
						}
					}
				}
