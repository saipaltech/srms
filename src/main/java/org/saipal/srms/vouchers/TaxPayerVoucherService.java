...
public class TaxPayerVoucherService extends AutoService {
	...
	public ResponseEntity<Map<String, Object>> index() {
		...
		Map<String, Object> result = p.setPageNo(request("page")).setPerPage(request("perPage")).setOrderBy(sort)
				.select("cast(id as char) as id,cast(date as date) as date,approved, voucherno,taxpayername,taxpayerpan,depositedby,depcontact,lgid,collectioncenterid,accountno,revenuecode,purpose,amount")
...
//				Arrays.asList(id));
//		data.put("revs", revs);
//		String sql = "select  top 1 *,cast(id as varchar) as iid,cast((format(getdate(),'yyyyMMdd')) as numeric) as today from "+table+" where voucherno=? and bankid=? and branchid=?";
					...
					if(d.getInt("isused")==1) {
						...
						if(pdata!=null) {
								...
								if(pdata.getInt("status")==1) {
									...
									throw new JSONException("Not able to update");
								}
							}else {
								throw new JSONException("Not able to update");
							}
