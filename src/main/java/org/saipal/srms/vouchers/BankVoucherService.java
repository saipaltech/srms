...
public class BankVoucherService extends AutoService {
	...
	public ResponseEntity<Map<String, Object>> chequeDeposit() throws JSONException {
		...
		if (jarr.length() > 0) {
			...
			String sql="insert into taxvouchers(id,bankid,karobarsanket,chequeno,chequeamount,cstatus,chequebank,lgid,date,taxpayername,bankorgid,amountcr,cstatus,ttype) values(?,?,?,?,?,?,?,?,?,?,?,?)";
			db.execute(sql,Arrays.asList(res.get("did"),res.get("bankid"),res.get("ksno"),res.get("chequeno"),res.get("chequeamount"),0,2));
			}
		}
		return null;
	}
