...
import { VerifyVoucherDirectDepositService } from './verify-voucher-direct-deposit.service';
import { ValidationService } from '../validation.service';
import { DatePipe } from '@angular/common';


@Component({
  selector: 'app-verify-voucher-direct-deposit',
  templateUrl: './verify-voucher-direct-deposit.component.html',
  styleUrls: ['./verify-voucher-direct-deposit.component.scss'],
  providers: [DatePipe]
})
export class VerifyVoucherDirectDepositComponent {
  ...
  constructor(private toastr: ToastrService, private fb: FormBuilder, private RS: VerifyVoucherDirectDepositService, private datePipe: DatePipe) {
...
model1:any;
  chequeFormSubmit(){
    if (window.confirm('Are  you sure you want to save ?')) {
      ...
      this.RS.submitDirectDeposit(this.model1).subscribe({
        next:(result:any) => {
          // this.lists=result.data;
    ...
    if (this.srchForm.valid) {
      this.RS.getTranactionData(this.srchForm.value.srch_term).subscribe({
        next: (dt) => {
          ...
          if(this.transDetails.trantype==8){
            this.cDt=true;
            this.bDt=false;
          }
          // else{
